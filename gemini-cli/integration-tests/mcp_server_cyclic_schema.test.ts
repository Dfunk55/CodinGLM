 ...
 * (in JSON, using the $ref keyword) and both the GenAI SDK and the CodinGLM
 ...
 * the CodinGLM API call would fail for such tools because the schema was
 * passed not as a JSON string but using the CodinGLM API's tool parameter
 * schema object which has stricter typing and recursion restrictions.
 * If this test fails, it's likely because either the GenAI SDK or CodinGLM API
 * has become more restrictive about the type of tool parameter schemas that
 * are accepted. If this occurs: CodinGLM CLI previously attempted to detect
 * such tools and proactively remove them from the set of tools provided in
 * the CodinGLM API call (as FunctionDeclaration objects). It may be appropriate
 * to resurrect that behavior but note that it's difficult to keep the
 * GCLI filters in sync with the CodinGLM API restrictions and behavior.
 */
