...
const installedLinters = new Set();
const skippedLinters = new Set();
const allowMissingLinters = process.env.CODINGLM_SKIP_LINTER_INSTALL === 'true';
...
export function setupLinters() {
  ...
  for (const linter in LINTERS) {
    ...
    if (runCommand(check, 'ignore')) {
      installedLinters.add(linter);
      continue;
    }
    console.log(`Installing ${linter}...`);
    if (!runCommand(installer)) {
      const message = `Failed to install ${linter}.`;
      if (allowMissingLinters) {
        console.warn(
          `${message} Skipping because CODINGLM_SKIP_LINTER_INSTALL is set.`,
        );
        skippedLinters.add(linter);
        continue;
      }
      console.error(`${message} Please install it manually.`);
      process.exit(1);
    }
    installedLinters.add(linter);
  }

  if (skippedLinters.size > 0) {
    console.warn(
      `Skipping the following linters for this run: ${Array.from(skippedLinters).join(', ')}`,
    );
  }
...
export function runActionlint() {
  if (!installedLinters.has('actionlint')) {
    console.warn('Skipping actionlint (not available).');
    return;
  }
  console.log('\nRunning actionlint...');
...
export function runShellcheck() {
  if (!installedLinters.has('shellcheck')) {
    console.warn('Skipping shellcheck (not available).');
    return;
  }
  console.log('\nRunning shellcheck...');
...
export function runYamllint() {
  if (!installedLinters.has('yamllint')) {
    console.warn('Skipping yamllint (not available).');
    return;
  }
  console.log('\nRunning yamllint...');
