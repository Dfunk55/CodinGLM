...
if (!image.length) {
  console.warn(
    'No default image tag specified in CodinGLM/packages/cli/package.json',
  );
}
...
console.log('packing @codinglm/cli ...');
const cliPackageDir = join('packages', 'cli');
rmSync(join(cliPackageDir, 'dist', 'codinglm-cli-*.tgz'), { force: true });
execSync(
  `npm pack -w @codinglm/cli --pack-destination ./packages/cli/dist`,
  {
    stdio: 'ignore',
  },
);

console.log('packing @codinglm/core ...');
const corePackageDir = join('packages', 'core');
rmSync(join(corePackageDir, 'dist', 'codinglm-core-*.tgz'), {
  force: true,
});
execSync(
  `npm pack -w @codinglm/core --pack-destination ./packages/core/dist`,
  { stdio: 'ignore' },
);
...
chmodSync(
  join(cliPackageDir, 'dist', `codinglm-cli-${packageVersion}.tgz`),
  0o755,
);
chmodSync(
  join(corePackageDir, 'dist', `codinglm-core-${packageVersion}.tgz`),
  0o755,
);
