...
CodinGLM CLI can integrate with your IDE to provide a more seamless and
...
- **Native Diffing:** When CodinGLM suggests code modifications, you can view the
...
- **VS Code Commands:** You can access CodinGLM CLI features directly from the VS
  Code Command Palette (`Cmd+Shift+P` or `Ctrl+Shift+P`):
  - `CodinGLM CLI: Run`: Starts a new CodinGLM CLI session in the integrated
    terminal.
  - `CodinGLM CLI: Accept Diff`: Accepts the changes in the active diff editor.
  - `CodinGLM CLI: Close Diff Editor`: Rejects the changes and closes the active
    diff editor.
  - `CodinGLM CLI: View Third-Party Notices`: Displays the third-party notices for
    the extension.
...
When you run CodinGLM CLI inside a supported editor, it will automatically detect
...
manually, you can run the following command inside CodinGLM CLI:
...
> NOTE: The "CodinGLM CLI Companion" extension may appear towards the bottom of
...
When enabled, CodinGLM CLI will automatically attempt to connect to the IDE
companion extension.
...
When you ask CodinGLM to modify a file, it can open a diff view directly in your
editor.
...
- Open the Command Palette and run **CodinGLM CLI: Accept Diff**.
...
- Open the Command Palette and run **CodinGLM CLI: Close Diff Editor**.
...
If you are using CodinGLM CLI within a sandbox, please be aware of the following:
...
- **On macOS:** The IDE integration requires network access to communicate with
  ...
  network access.
- **In a Docker Container:** If you run CodinGLM CLI inside a Docker (or Podman)
...
- **Message:**
  ...
  - **Cause:** CodinGLM CLI could not find the necessary environment variables
  ...
  - **Solution:**
    1.  Make sure you have installed the **CodinGLM CLI Companion** extension in
...
- **Message:**
  `🔴 Disconnected: Directory mismatch. CodinGLM CLI is running in a different location than the open workspace in [IDE Name]. Please run the CLI from one of the following directories: [List of directories]`
...
- **Message:**
  `IDE integration is not supported in your current environment. To use this feature, run CodinGLM CLI in one of these supported IDEs: [List of IDEs]`
  - **Cause:** You are running CodinGLM CLI in a terminal or environment that is
    not a supported IDE.
  - **Solution:** Run CodinGLM CLI from the integrated terminal of a supported
    IDE, like VS Code.

- **Message:**
  `No installer is available for IDE. Please install the CodinGLM CLI Companion extension manually from the marketplace.`
  - **Cause:** You ran `/ide install`, but the CLI does not have an automated
    installer for your specific IDE.
  - **Solution:** Open your IDE's extension marketplace, search for "CodinGLM CLI
    Companion", and
