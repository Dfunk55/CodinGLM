...
The CodinGLM CLI includes built-in tools that the GLM model uses to interact
...
## Overview of CodinGLM CLI tools

In the context of the CodinGLM CLI, tools are specific functions or modules that
the GLM model can request to be executed. For example, if you ask CodinGLM to
...
definitions (schemas) to the GLM model, executes them when requested, and
returns the results to the model for further processing into a user-facing
response.

These tools provide the following capabilities:

- **Access local information:** Tools allow CodinGLM to access your local file
  system, read file contents, list directories, etc.
- **Execute commands:** With tools like `run_shell_command`, CodinGLM can run
...
- **Ground responses:** By using tools to fetch real-time or specific local
  data, CodinGLM's responses can be more accurate, relevant, and grounded in your
  actual context.

## How to use CodinGLM CLI tools

To use CodinGLM CLI tools, provide a prompt to the CodinGLM CLI. The process works
as follows:

1.  You provide a prompt to the CodinGLM CLI.
2.  The CLI sends the prompt to the core.
3.  The core, along with your prompt and conversation history, sends a list of
    available tools and their descriptions/schemas to the CodinGLM API.
4.  The GLM model analyzes your request. If it determines that a tool is
...
6.  The output from the tool is sent back to the GLM model.
7.  The GLM model uses the tool's output to formulate its final answer, which
...
in mind. The CodinGLM CLI will typically:
...
- **Utilize sandboxing:** All tools are subject to restrictions enforced by
  sandboxing (see [Sandboxing in the CodinGLM CLI](../cli/sandbox.md)). This means
...
## Learn more about CodinGLM CLI's tools

CodinGLM CLI's built-in tools can be broadly categorized as follows:
...
- **[MCP servers](./mcp-server.md)**: MCP servers act as a bridge between the
  GLM model and your local environment or other services like APIs.
