# CodinGLM CLI file system tools

The CodinGLM CLI provides a comprehensive suite of tools for interacting with the
local file system. These tools allow the GLM model to read from, write to,
...
- **Behavior:**
  ...
  - **Enhanced Reliability (Multi-Stage Edit Correction):** To significantly
    ...
    - If the initial `old_string` isn't found or matches multiple locations, the
      tool can leverage the GLM model to iteratively refine `old_string` (and
...
These file system tools provide a foundation for the CodinGLM CLI to understand
