...
CodinGLM CLI supports a variety of themes to customize its color scheme and
appearance. You can change the theme to suit your preferences via the `/theme`
command or `"theme":` configuration setting.

## Available Themes

CodinGLM CLI comes with a selection of pre-defined themes, which you can list
using the `/theme` command within CodinGLM CLI:
...
- **Light Themes:**
  ...
  - `Default Light`
  - `GitHub Light`
- `Classic Code` (legacy "googlecode")
  - `Xcode`

### Changing Themes

1.  Enter `/theme` into CodinGLM CLI.
...
Selected themes are saved in CodinGLM CLI's
...
CodinGLM CLI allows you to create your own custom color themes by specifying them
...
**Security Note:** For your safety, CodinGLM CLI will only load theme files that
...
- Select your custom theme using the `/theme` command in CodinGLM CLI. Your custom
...
<img src="/assets/theme-google-light.png" alt="Classic Code theme" width="600">
