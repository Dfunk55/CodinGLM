...
prompts as personal shortcuts within CodinGLM CLI. You can create commands that
...
CodinGLM CLI discovers commands from two locations, loaded in a specific order:
...
- `prompt` (String): The prompt that will be sent to the GLM model when the
...
When a custom command attempts to execute a shell command, CodinGLM CLI will now
...
CodinGLM CLI will then execute the multi-line prompt defined in your TOML file.
