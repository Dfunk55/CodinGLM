# Provide Context with CODINGLM.md Files

Context files, which use the default name `CODINGLM.md`, are a powerful feature
for providing instructional context to the GLM model. You can use these files
...
1.  **Global context file:**
    - **Location:** `~/.gemini/CODINGLM.md` (in your user home directory).
    - **Scope:** Provides default instructions for all your projects.

2.  **Project root and ancestor context files:**
    - **Location:** The CLI searches for a `CODINGLM.md` file in your current
...
3.  **Sub-directory context files:**
    - **Location:** The CLI also scans for `CODINGLM.md` files in subdirectories
...
### Example `CODINGLM.md` file

Here is an example of what you can include in a `CODINGLM.md` file at the root of
...
- **`/memory refresh`**: Forces a re-scan and reload of all `CODINGLM.md` files
  from all configured locations.
- **`/memory add <text>`**: Appends your text to your global
  `~/.gemini/CODINGLM.md` file. This lets you add persistent memories on the fly.

## Modularize context with imports

You can break down large `CODINGLM.md` files into smaller, more manageable
...
**Example `CODINGLM.md` with imports:**

```markdown
# Main CODINGLM.md file
...
While `CODINGLM.md` is the default filename, you can configure this in your
...
{
  "context": {
    "fileName": ["AGENTS.md", "CONTEXT.md", "CODINGLM.md"]
  }
}
```
