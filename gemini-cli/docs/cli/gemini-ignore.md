...
This document provides an overview of the CodinGLM Ignore (`.geminiignore`)
feature of the CodinGLM CLI.

The CodinGLM CLI includes the ability to automatically ignore files, similar to
`.gitignore` (used by Git) and `.aiexclude` (used by CodinGLM Code Assist). Adding
...
must restart your CodinGLM CLI session.
