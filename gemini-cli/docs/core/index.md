...
CodinGLM CLI's core package (`packages/core`) is the backend portion of CodinGLM
CLI, handling communication with the CodinGLM API, managing tools, and processing
requests sent from `packages/cli`. For a general overview of CodinGLM CLI, see the
...
- **[Memory Import Processor](./memport.md):** Documentation for the modular
  CODINGLM.md import feature using @file.md syntax.
- **[Policy Engine](./policy-engine.md):** Use the Policy Engine for
  fine-grained control over tool execution.

## Role of the core

While the `packages/cli` portion of CodinGLM CLI provides the user interface,
`packages/core` is responsible for:

- **GLM API interaction:** Securely communicating with the Z.AI GLM endpoints,
  sending user prompts, and receiving model responses.
- **Prompt engineering:** Constructing effective prompts for the GLM model,
  potentially incorporating conversation history, tool definitions, and
  instructional context from `CODINGLM.md` files.
- **Tool management & orchestration:**
  - Registering available tools (e.g., file system tools, shell command
    execution).
  - Interpreting tool use requests from the GLM model.
  - Executing the requested tools with the provided arguments.
  - Returning tool execution results to the GLM model for further processing.
...
- **API key management:** It handles the `GEMINI_API_KEY` and ensures it's used
  securely when communicating with the CodinGLM API.
...
To ensure that long conversations don't exceed the token limits of the CodinGLM
...
[GLM-4.6 model card](../../GLM-4.6_MODEL_CARD.md) or on the
[Zhipu AI docs](https://open.bigmodel.cn/pricing).

## Model fallback

CodinGLM CLI includes a model fallback mechanism to ensure that you can continue
...
`CODINGLM.md` files that provide context to the model. It searches for these files
...
`refresh` the content of loaded `CODINGLM.md` files.

## Citations

When CodinGLM finds it is reciting text from a source it appends the citation to
