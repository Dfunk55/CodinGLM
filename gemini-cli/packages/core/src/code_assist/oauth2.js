...
// Note: CodinGLM uses Z.AI API key authentication, not Google OAuth.
// These credentials are placeholders from the original gemini-cli code.
const OAUTH_CLIENT_ID = 'REMOVED-FOR-CODINGLM';
// OAuth Secret value used to initiate OAuth2Client class.
// Note: CodinGLM uses Z.AI API key authentication, not Google OAuth.
// This credential is a placeholder from the original gemini-cli code.
const OAUTH_CLIENT_SECRET = 'REMOVED-FOR-CODINGLM';
...
async function initOauthClient(authType, config) {
    ...
    if (config.isBrowserLaunchSuppressed()) {
        ...
        try {
            ...
            debugLogger.error(`Failed to open browser with error:`, getErrorMessage(err), `\nPlease try running again with NO_BROWSER=true set.`);
