            ...
            onConfirm: async (outcome) => {
                ...
                if (ideConfirmation) {
                    ...
                    if (result.status === 'accepted' && result.content) {
                        // NOTE: Potential optimization - See https://github.com/google-gemini/gemini-cli/pull/5618#discussion_r2255413084
                        // This handles a possible race condition where the file is modified on disk while being edited.
                        // Current implementation is safe and handles this correctly.
...
export class SmartEditTool extends BaseDeclarativeTool {
    ...
    constructor(config, messageBus) {
        super(SmartEditTool.Name, 'Edit', `Replaces text within a file. Replaces a single occurrence. This tool requires providing significant context around the change to ensure precise targeting. Always use the ${READ_FILE_TOOL_NAME} tool to examine the file's current content before attempting a text replacement.
