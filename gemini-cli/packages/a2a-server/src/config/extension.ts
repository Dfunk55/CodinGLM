 ...
 * CodinGLMExtension class defined in Core.
 */
...
export function loadExtensions(workspaceDir: string): CodinGLMExtension[] {
  const allExtensions = [
    ...loadExtensionsFromDir(workspaceDir),
    ...loadExtensionsFromDir(os.homedir()),
  ];

  const uniqueExtensions: CodinGLMExtension[] = [];
...
function loadExtensionsFromDir(dir: string): CodinGLMExtension[] {
  const extensionsDir = path.join(dir, EXTENSIONS_DIRECTORY_NAME);
  if (!fs.existsSync(extensionsDir)) {
    return [];
  }

  const extensions: CodinGLMExtension[] = [];
...
function loadExtension(extensionDir: string): CodinGLMExtension | null {
