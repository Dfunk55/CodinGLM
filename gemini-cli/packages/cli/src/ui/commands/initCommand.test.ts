...
describe('initCommand', () => {
  ...
  const contextFilePath = path.join(targetDir, 'CODINGLM.md');
  ...
  it('should inform the user if CODINGLM.md already exists', async () => {
    ...
    expect(result).toEqual({
      ...
      messageType: 'info',
      content:
        'A CODINGLM.md file already exists in this directory. No changes were made.',
    });
    // Assert: Ensure no file was written
    expect(fs.writeFileSync).not.toHaveBeenCalled();
  });

  it('should create CODINGLM.md and submit a prompt if it does not exist', async () => {
    ...
    expect(mockContext.ui.addItem).toHaveBeenCalledWith(
      {
        type: 'info',
        text: 'Empty CODINGLM.md file created. Now analyzing the project to populate it.',
      },
      expect.any(Number),
    );
    ...
    expect(result.content).toContain(
      'You are an AI agent that brings the power of CodinGLM',
    );
  });
