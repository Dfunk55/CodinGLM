...
import { debugLogger, type Config } from '@codinglm/core';
  ...
  return async () => {
    ...
    if (currentMemory && currentMemory.trim().length > 0) {
      ...
      addMessage({
        ...
        content:
          fileCount > 0
            ? 'Hierarchical memory (CODINGLM.md or other context files) is loaded but content is empty.'
            : 'No hierarchical memory (CODINGLM.md or other context files) is currently loaded.',
        timestamp: new Date(),
      });
    }
  };
