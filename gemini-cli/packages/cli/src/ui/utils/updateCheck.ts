...
import { getPackageJson, debugLogger } from '@codinglm/core';
  ...
  try {
    ...
    if (isNightly) {
      ...
      if (bestUpdate && semver.gt(bestUpdate, currentVersion)) {
        const message = `A new version of CodinGLM is available! ${currentVersion} → ${bestUpdate}`;
      ...
      if (latestUpdate && semver.gt(latestUpdate, currentVersion)) {
        const message = `CodinGLM update available! ${currentVersion} → ${latestUpdate}`;
