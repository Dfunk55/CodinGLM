...
import type { Config, SandboxConfig } from '@codinglm/core';
import {
  coreEvents,
  debugLogger,
  FatalSandboxError,
...
export async function start_sandbox(
  ...
  try {
    ...
    if (process.env['Z_AI_API_KEY']) {
      args.push('--env', `Z_AI_API_KEY=${process.env['Z_AI_API_KEY']}`);
    }
    ...
    if (process.env['Z_AI_BASE_URL']) {
      args.push('--env', `Z_AI_BASE_URL=${process.env['Z_AI_BASE_URL']}`);
    }
    if (process.env['ZAI_BASE_URL']) {
      args.push('--env', `ZAI_BASE_URL=${process.env['ZAI_BASE_URL']}`);
    }
